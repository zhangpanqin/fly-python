...
if __name__ == '__main__':
    ...
    # 源 Spreadsheet
    source_spreadsheet_id = '1CvqM1M95hhQn1jYlIzVEPt96pu96SlLuLh0m1vS0xVc'
    # source_spreadsheet_id = '1iik2FUYfRveuuoBISdSCaDOwWdw0GVncjC6zkGPmhdw'
    # source_spreadsheet_id = '16zn7zMI1T1N8__0aU9SKW46xxLpSjFgPl2_YoXO4Odo'
    ...
    # 创建目标 Spreadsheet
    target_spreadsheet = client.copy(source_spreadsheet_id, title='Archived_1223' + source_spreadsheet.title)
    ...
    for source_sheet in source_spreadsheet.worksheets():
        ...
        if len(cell_list) > 0:
            target_sheet.update_cells(cell_list, value_input_option=ValueInputOption.raw)

        # for row_index, formulas_row in enumerate(source_formulas):
        #     for col_index, cell_formula in enumerate(formulas_row):
        #         if isinstance(cell_formula, str) and cell_formula.formula and not cell_formula.startswith('=IMPORTRANGE'):
        #             target_sheet.update_cell(row_index + 1, col_index + 1, cell_formula)

        logging.info("结束处理: " + source_sheet_title)

    target_spreadsheet.share('huxhu@thoughtworks.com', perm_type='user', role='writer')
